...
import yaml
from capstone import CS_ARCH_RISCV, CS_MODE_32, Cs

from backends.generators.generator import load_csrs
...
def get_pseudo_instr():
    csrrs_path = f"{os.path.dirname(__file__)}/../../../spec/std/isa/inst/Zicsr/csrrs.yaml"
...
def main():
    parser = argparse.ArgumentParser(description="Compare CSR switch cases in two C files.")
    parser.add_argument(
        "--csr_switch",
    ...
    diff = {
        t
        for t in diff
        if not (t[0] in pseudo_instr_to_csr and (pseudo_instr_to_csr[t[0]], t[1]) in cases_gen)
    }
